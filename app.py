...
def create_pdf(report_data, raw_text, doc_name):
    ...
    # Detailed Metrics Section (Premium List)
    pdf.set_fill_color(236, 240, 241); pdf.set_font('Arial', 'B', 12); pdf.set_text_color(44, 62, 80)
    pdf.cell(180, 8, ' 2. PERFORMANCE METRICS & ANALYSIS', 0, 1, 'L', fill=True); pdf.ln(4)
    
    metrics = [
        ("A. ATS Parsability", f"{report_data['parsability_score']}%"),
        ("B. Skor XYZ (Google Standard)", f"{int(report_data['xyz_score'])}%"),
        ("C. Kuantifikasi Metrik", f"{report_data['metrics_count']} Data"),
        ("D. Estimasi Masa Kerja", f"{report_data['total_tenure']} Tahun"),
        ("E. Format Dokumen", f"{report_data['pages']} Halaman")
    ]
    for t, v in metrics:
    ...
    pdf.ln(5); pdf.set_fill_color(236, 240, 241); pdf.set_font('Arial', 'B', 12); pdf.cell(180, 8, ' 3. DIAGNOSTIC RESULTS', 0, 1, 'L', fill=True); pdf.ln(4)
    
    diag_items = [
        ("STRUCTURE", report_data['missing_sections'] == [], f"Bagian Hilang: {', '.join(report_data['missing_sections'])}" if report_data['missing_sections'] else "Seluruh 4 pilar wajib terdeteksi."),
        ("CONTENT", report_data['xyz_score'] >= 50, "Gunakan Action Verb + Metrik." if report_data['xyz_score'] < 50 else "Penggunaan XYZ Formula sudah baik."),
        ("CONTACT", all(report_data['contact_info'].values()), f"Item Terdeteksi: {sum(report_data['contact_info'].values())}/4 (Email, Phone, LinkedIn, Domisili).")
    ]
    for label, is_ok, msg in diag_items:
        bg = (234, 250, 241) if is_ok else (253, 237, 236)
        text_c = (39, 174, 96) if is_ok else (192, 57, 43)
        status_tag = "[EXCELLENT]" if is_ok else "[ACTION NEEDED]"
        pdf.set_fill_color(*bg); pdf.set_text_color(*text_c); pdf.set_font('Arial', 'B', 10)
        pdf.cell(180, 6, f" {status_tag} - {label}", 0, 1, fill=True)
        pdf.set_text_color(40, 40, 40); pdf.set_font('Arial', '', 10)
        pdf.multi_cell(180, 5, msg, fill=True); pdf.ln(3)
...
with st.sidebar:
    ...
    st.markdown("### 🚦 System: **Online**"); st.divider(); st.markdown("<small>v3.8.0 Premium</small>", unsafe_allow_html=True)
...
if uploaded_file:
    with st.status("🔍 Menganalisis Dokumen...", expanded=True) as status:
        ...
        tab1, tab2, tab3 = st.tabs(["📊 Diagnostic", "📤 Export PDF", "🛠️ Raw"])
        with tab1:
            st.markdown("##### 📌 ATS Keywords"); st.write(", ".join(res['top_keywords']))
            st.divider(); st.info(f"Verb Terdeteksi: {', '.join(res['extracted_verbs'])}")
        with tab2:
